...
3. 波形・特徴量・ブロック情報が表示される
4. 画面下部の **「↻ 更新」** でブロックチェーン一覧を更新
...
|---------|---------|--------------|
| Python | 24 | pytest |
| Node.js | 39 | Jest + supertest |
| Vue.js | 30 | Vitest + @vue/test-utils |
| **合計** | **93** | |
...
`mockup.html` をブラウザで直接開くと、APIなしでUIの動作確認ができます。

---

## 設計ドキュメント

`/Users/mac/dev/01 doc/project/Nodejs/sound-blockchain/` に格納しています。

| ファイル | 内容 |
|---------|------|
| `01_要件定義書.md` | 機能要件・非機能要件・制約条件・用語集 |
| `02_基本設計書.md` | システム構成・API設計・データフロー・ブロックチェーン設計 |
| `03_詳細設計書.md` | 各ファイルの詳細仕様・起動手順 |
| `04_テスト設計書.md` | テスト方針・全テストケース一覧・実行手順 |
