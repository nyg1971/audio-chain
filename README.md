...
#### 音声解析（Python）・Webアプリ（Node.js / Vue）・ブロックチェーン概念を組み合わせた真正性検証のPoC
...
本プロトタイプでは、3層の登録・記録フローを実装・検証しました。この記録データを基盤に「登録済み音源との照合・検証フロー」を組み合わせることで、音声コンテンツの真正性を担保する技術になると考えています。
...
- SHA-256（バイト同一性）と音声特徴量MFCC（コンテンツ同一性）を組み合わせた2段階でのブロックチェーン記録
